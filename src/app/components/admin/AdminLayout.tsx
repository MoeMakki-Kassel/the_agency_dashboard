...
export function AdminLayout() {
  ...
  return (
    <div className="flex h-screen bg-[#e8e8e8] text-[#000000] font-['Inter']">
      {mobileMenuOpen && (
        <div
      ...
      <aside
        className={`fixed inset-y-0 start-0 z-50 w-[min(100vw-3rem,260px)] sm:w-[260px] bg-[#000000] text-[#e8e8e8] flex flex-col transition-transform duration-300 ease-in-out lg:relative ${sidebarTransform}`}
      >
        <div className="p-6 flex items-center justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="text-xs text-[#8c8c8c] mt-2 uppercase tracking-wider font-semibold">
          ...
          {visibleNavItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <Icon size={20} className={isActive ? "text-white" : "text-[#8c8c8c]"} />
        ...
        <div className="p-4 border-t border-white/10 shrink-0">
          ...
          >
            <LogOut size={20} className="text-[#8c8c8c]" />
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="min-h-[72px] bg-white border-b border-[#e8e8e8] flex flex-wrap items-center justify-between gap-3 px-4 lg:px-8 py-2 lg:py-0 shrink-0">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <button
              type="button"
              className="lg:hidden p-2 -ms-2 text-[#8c8c8c] hover:text-[#000000] transition-colors shrink-0"
            ...
            <div className="max-w-md w-full relative flex-1 min-w-0">
              ...
              <input
                ...
                className={`w-full bg-[#e8e8e8] border-none rounded-full py-2 text-sm focus:ring-2 focus:ring-[#000000] outline-none ${
          ...
          <div className="flex items-center gap-2 sm:gap-3 lg:gap-6 shrink-0 ms-auto">
            <div
              className="flex rounded-lg overflow-hidden border border-[#e8e8e8] bg-[#e8e8e8]/80 shrink-0"
              role="group"
            ...
            >
              <button
                ...
                className={`px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm font-semibold transition-colors ${
                  language === 'EN' ? 'bg-white text-black shadow-sm' : 'text-[#525252] hover:bg-white/80'
                }`}
              >
              ...
              <button
                ...
                className={`px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm font-semibold transition-colors ${
                  language === 'AR' ? 'bg-white text-black shadow-sm' : 'text-[#525252] hover:bg-white/80'
                }`}
              >
                {t('admin.shell.lang_ar')}
              </button>
            </div>
            <button type="button" className="relative text-[#8c8c8c] hover:text-[#000000] transition-colors p-1">
              <Bell size={24} />
              <span className="absolute top-0 end-0 w-2.5 h-2.5 bg-[#525252] rounded-full border-2 border-white" />
            </button>
            <button
              type="button"
              className="flex items-center gap-3 border-s border-[#e8e8e8] ps-3 lg:ps-6 cursor-pointer hover:bg-gray-50 rounded-lg p-1 text-start"
              onClick={() => {
                signOut();
                window.location.href = '/login';
              }}
            >
              ...
              <div className="hidden sm:block min-w-0">
                ...
                <div className="text-xs text-[#8c8c8c] truncate">{user?.email ?? t('admin.shell.logout')}</div>
              </div>
            </button>
          </div>
        </header>
