...
export function VenueMapViewer({ layout, tierConfig }: VenueMapViewerProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4">
      <div className="lg:w-56 shrink-0 space-y-3 bg-white border border-[#e8e8e8] rounded-xl p-4">
        <p className="text-xs font-medium text-[#000000]">View only</p>
        ...
        <div className="border-t pt-3 space-y-1">
          ...
          <p className="text-xs text-[#8c8c8c] pt-1">Total map seats: {seats.length}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-2 min-h-[480px]">
        ...
        >
          ...
          {!floorPlanUrl && (
            <p className="text-sm text-[#8c8c8c] p-4">No floor plan image on this template.</p>
          )}
          <svg
            ref={svgRef}
            viewBox={view.join(' ')}
