...
export function AdminSettings() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      <div>
        <h1 className="text-3xl font-bold font-['Tajawal'] text-foreground">Platform Settings</h1>
        <p className="text-muted-foreground mt-1">Configure global preferences, contact details, and platform defaults.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mt-8">
        {/* Sidebar Nav */}
        <div className="lg:col-span-1 space-y-1 bg-card p-4 rounded-xl border border-border shadow-[0_8px_24px_rgba(20,14,8,0.04)]">
          ...
          <div className="bg-card rounded-xl shadow-[0_8px_24px_rgba(20,14,8,0.04)] border border-border overflow-hidden">
            ...
            <div className="p-6">
              {activeTab === "general" && (
                <div className="space-y-6 max-w-2xl">
                  <div className="space-y-2">
                    ...
                    {isLoading ? (
                      ...
                      <input
                        ...
                        className="w-full bg-muted border border-transparent rounded-lg py-2.5 px-4 text-sm focus:bg-card focus:border-primary outline-none"
                        placeholder="TheAgencyJo"
                      />
                    )}
                    <p className="text-xs text-muted-foreground">Displayed in the browser tab and frontend header.</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">Direct URL to the platform logo image (for light backgrounds, PDFs, and email).</p>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-foreground">Logo URL (dark backgrounds)</label>
                    {isLoading ? (
                      <FieldSkeleton />
                    ) : (
                      <input
                    ...
                    <p className="text-xs text-muted-foreground">Optional light / inverted logo for dark sections (e.g. hero). If empty, the site may auto-invert the default logo where needed.</p>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-foreground">Home — Watch reel URL</label>
                    {isLoading ? (
                      <FieldSkeleton />
                    ) : (
                      <input
                    ...
                    <p className="text-xs text-muted-foreground">Instagram, TikTok, or other reel link for the &quot;Watch Reel&quot; button on the public home hero. Leave empty to hide that button.</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">Default currency shown in the dashboard and frontend.</p>
                  </div>
              ...
              {activeTab === "contact" && (
                <div className="space-y-8 max-w-2xl">
                  <div className="space-y-4">
                    <h3 className="font-bold text-foreground flex items-center gap-2 border-b border-border pb-2">
                      <Phone size={18} className="text-foreground shrink-0" /> Primary Phone Numbers
                    </h3>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="block text-sm font-medium text-foreground">Line 1 Label</label>
                        {isLoading ? (
                          <FieldSkeleton />
                        ) : (
                          <input
                      ...
                      <div className="space-y-2 md:col-span-2">
                        <label className="block text-sm font-medium text-foreground">Line 1 Number</label>
                        {isLoading ? (
                          <FieldSkeleton />
                        ) : (
                          <PhoneCountryField
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="block text-sm font-medium text-foreground">Line 2 Label</label>
                        {isLoading ? (
                          <FieldSkeleton />
                        ) : (
                          <input
                      ...
                      <div className="space-y-2">
                        <label className="block text-sm font-medium text-foreground">Line 2 Number</label>
                        {isLoading ? (
                          <FieldSkeleton />
                        ) : (
                          <PhoneCountryField
                  ...
                  <div className="space-y-4 pt-4">
                    <h3 className="font-bold text-foreground flex items-center gap-2 border-b border-border pb-2">
                      <MessageCircle size={18} className="text-muted-foreground shrink-0" /> WhatsApp Integration
                    </h3>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">Used for "Chat on WhatsApp" links.</p>
                    </div>
                      ...
                      {isLoading ? (
                        ...
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                            contact.contact_whatsapp_enabled ? "bg-primary" : "bg-muted"
                          }`}
                        >
                          <span
                            className={`inline-block h-4 w-4 rounded-full bg-card transition-[margin] ${
                              contact.contact_whatsapp_enabled ? "ms-6" : "ms-1"
                            }`}
                          />
                        </button>
                      )}
                      <span className="text-sm text-foreground">Enable WhatsApp button on the frontend</span>
                    </div>
                  </div>

                  <div className="space-y-4 pt-4">
                    <h3 className="font-bold text-foreground flex items-center gap-2 border-b border-border pb-2">
                      <Mail size={18} className="text-foreground shrink-0" /> Email Support
                    </h3>
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-foreground">Support Email Address</label>
                      {isLoading ? (
                        <FieldSkeleton />
                      ) : (
                        <input
                  ...
                  <div className="space-y-4 pt-4">
                    ...
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-foreground">Contact page — Visit card</label>
                      {isLoading ? (
                        <FieldSkeleton />
                      ) : (
                        <textarea
                          ...
                          rows={4}
                          className="w-full bg-muted border border-transparent rounded-lg py-2.5 px-4 text-sm focus:bg-card focus:border-primary outline-none resize-y min-h-[100px]"
                          placeholder="Street, city, country (shown on the public Contact page)"
                        />
                      )}
                      <p className="text-xs text-muted-foreground">Shown on the public site Contact page. Leave empty to hide the visit block.</p>
                    </div>
                  </div>
              ...
              {activeTab === "footer" && (
                <div className="space-y-6 max-w-2xl">
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-foreground">Instagram URL</label>
                    {isLoading ? (
                      <FieldSkeleton />
                    ) : (
                      <input
              ...
              {activeTab === "brand" && (
                <div className="space-y-6 max-w-2xl">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">Display name used in emails, OG metadata, and copyright lines (when wired up).</p>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-foreground">Brand Website URL</label>
                    {isLoading ? <FieldSkeleton /> : (
                      <input
                        type="url"
                    ...
                    <p className="text-xs text-muted-foreground">Shown on the NI payment checkout under the merchant logo. Customers click this if they want more info before paying.</p>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-foreground">Brand Instagram URL</label>
                    {isLoading ? <FieldSkeleton /> : (
                      <input
                        type="url"
                    ...
                    <p className="text-xs text-muted-foreground">Separate from Footer URLs → Instagram (which is per-region marketing). This is the official brand handle.</p>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-foreground">Brand Contact Email</label>
                    {isLoading ? <FieldSkeleton /> : (
                      <input
                        type="email"
                    ...
                    <p className="text-xs text-muted-foreground">Primary public address. Used in schema.org / OG metadata. Per-region addresses live in the Global Contact Info tab.</p>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-foreground">NI Merchant Name</label>
                    {isLoading ? <FieldSkeleton /> : (
                      <input
                        type="text"
                    ...
                    <p className="text-xs text-muted-foreground">Merchant name shown on the NI hosted checkout page (uppercase, ≤25 chars).</p>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-foreground">NI Merchant Logo URL</label>
                    {isLoading ? <FieldSkeleton /> : (
                      <input
                        type="url"
                    ...
                    <p className="text-xs text-muted-foreground">Absolute URL to a hosted logo image (PNG/SVG). Shown on the NI checkout. If empty, falls back to the platform logo from the General tab.</p>
                  </div>
