...
export function AdminUsers() {
  ...
  const getInitialsBg = (role: string) => {
    if (role === 'super_admin') return 'bg-[#000000] text-[#e8e8e8]';
    if (role === 'secretary') return 'bg-primary text-primary-foreground';
    if (role === 'doorman') return 'bg-[#525252] text-white';
    return 'bg-[#e8e8e8] text-[#8c8c8c]';
  };

  const getRoleBadge = (role: string) => {
    if (role === 'super_admin') return 'bg-[#000000]/10 text-[#000000]';
    if (role === 'secretary') return 'bg-[#000000]/10 text-[#000000]';
    if (role === 'doorman') return 'bg-[#525252]/10 text-[#525252]';
    return 'bg-[#e8e8e8] text-[#8c8c8c]';
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold font-['Tajawal'] text-[#000000]">{t('admin.users.title')}</h1>
          <p className="text-[#8c8c8c] mt-1">{t('admin.users.subtitle')}</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          {isSuperAdmin && (
            <button
              type="button"
              onClick={openCreate}
              className="px-4 py-2 bg-black text-white rounded-lg text-sm font-medium hover:bg-accent transition-colors flex items-center gap-2 justify-center"
            >
              <Plus size={16} />
          ...
          <button
            type="button"
            onClick={exportUsers}
            className="px-4 py-2 border border-[#e8e8e8] bg-white rounded-lg text-sm font-medium hover:bg-[#e8e8e8] transition-colors flex items-center gap-2 justify-center"
          >
            <Download size={16} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-[0_8px_24px_rgba(20,14,8,0.04)] border border-[#e8e8e8] flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-[#000000]/10 flex items-center justify-center text-[#000000] shrink-0">
            <Users size={24} />
          </div>
          <div>
            <p className="text-sm font-medium text-[#8c8c8c]">{t('admin.users.total_customers')}</p>
            <h3 className="text-2xl font-bold text-[#000000] font-['Space_Grotesk']">
        ...
        <div className="bg-white p-6 rounded-xl shadow-[0_8px_24px_rgba(20,14,8,0.04)] border border-[#e8e8e8] flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-[#8c8c8c]">{t('admin.users.admins_managers')}</p>
            <h3 className="text-2xl font-bold text-[#000000] font-['Space_Grotesk']">
              {adminsAndManagers}
            </h3>
          </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-[0_8px_24px_rgba(20,14,8,0.04)] border border-[#e8e8e8] flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-[#525252]/10 flex items-center justify-center text-[#525252] shrink-0">
            <UserCheck size={24} />
          </div>
          <div>
            <p className="text-sm font-medium text-[#8c8c8c]">{t('admin.users.door_scanners')}</p>
            <h3 className="text-2xl font-bold text-[#000000] font-['Space_Grotesk']">
              {doorScanners}
            </h3>
          </div>
          ...
          <div className="flex w-full md:w-auto gap-3 flex-col sm:flex-row">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none w-full sm:w-auto px-4 py-2 border border-[#e8e8e8] bg-white rounded-lg text-[#000000] font-medium hover:bg-[#e8e8e8] transition-colors pe-8 focus:ring-2 focus:ring-[#000000] outline-none"
          ...
          <table className="w-full min-w-[62rem] table-fixed border-collapse admin-table text-sm">
            ...
            <tbody className="divide-y divide-[#e8e8e8]">
                ...
                <tr key={`skeleton-${i}`} className="animate-pulse">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="h-4 bg-[#e8e8e8] rounded w-28 mb-1"></div>
                        <div className="h-3 bg-[#e8e8e8] rounded w-40"></div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4"><div className="h-4 bg-[#e8e8e8] rounded w-24"></div></td>
                  <td className="px-6 py-4"><div className="h-6 bg-[#e8e8e8] rounded-full w-20"></div></td>
                  <td className="px-6 py-4"><div className="h-4 bg-[#e8e8e8] rounded w-14"></div></td>
                  <td className="px-6 py-4"><div className="h-4 bg-[#e8e8e8] rounded w-24"></div></td>
              ...
              {!isLoading && filteredUsers.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-[#8c8c8c]">
                    {t('admin.users.none')}
                  </td>
                </tr>
              )}
              ...
              {!isLoading && filteredUsers.map((user) => {
                ...
                return (
                  <tr key={user.id} className="hover:bg-[#e8e8e8]/50 transition-colors cursor-pointer">
                    <td className="px-6 py-4 align-top text-start min-w-0">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="font-medium text-[#000000] truncate">{user.first_name} {user.last_name}</div>
                    ...
                    <td className="px-6 py-4 align-top text-start text-[#525252] tabular-nums whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 align-top text-start">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full shrink-0 ${user.status === 'active' ? 'bg-[#525252]' : 'bg-[#8c8c8c]'}`}></div>
                        <span className={user.status === 'active' ? 'text-[#000000]' : 'text-[#8c8c8c]'}>{displayStatus}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 align-top text-start text-[#8c8c8c] tabular-nums">{formatDate(user.created_at)}</td>
                    <td className="px-6 py-4 align-top text-end">
                      <div className="relative inline-block text-start">
                        ...
                        {menuOpenId === user.id && (
                          <div className="absolute end-0 mt-2 w-56 bg-white rounded-lg shadow-xl border border-[#e8e8e8] py-1 z-20 animate-in fade-in zoom-in-95 duration-200">
                            {isSuperAdmin && (
                              <button
                                type="button"
                                onClick={(e) => openEdit(user, e)}
                                className="w-full flex items-center px-4 py-2 text-sm text-[#000000] hover:bg-[#e8e8e8] gap-2"
                              >
                                <Edit2 size={14} className="shrink-0" />
                                {t('admin.users.edit_user')}
                              </button>
                            )}
                            <div className="px-4 py-1.5 text-xs text-[#8c8c8c] font-semibold uppercase tracking-wider border-b border-[#e8e8e8] mb-1">
                            ...
                            {(rolesData?.data?.length
                              ...
                              <button
                                ...
                                className={`w-full flex items-center px-4 py-2 text-sm hover:bg-[#e8e8e8] gap-2 ${user.role === r.slug ? 'font-bold text-[#000000]' : 'text-[#000000]'}`}
                              >
        ...
        <AlertDialogContent className="border-[#e8e8e8] bg-white text-[#000000] sm:max-w-md">
          <AlertDialogHeader className="text-start sm:text-start">
            ...
            <AlertDialogDescription className="text-[#525252]">
              ...
              {deleteTarget && (
                <span className="block mt-2 font-medium text-[#000000]">
          ...
          <AlertDialogFooter className="gap-2 sm:justify-end flex-row flex-wrap">
            <AlertDialogCancel className="border-[#e8e8e8]">{t('admin.common.cancel')}</AlertDialogCancel>
            <button
              type="button"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 flex flex-col max-h-[80vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8e8e8]">
          <div>
            <h2 className="text-lg font-bold text-[#000000]">Assign Events</h2>
            <p className="text-sm text-[#8c8c8c]">{doorman.name}</p>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-[#8c8c8c] hover:text-[#000000] hover:bg-[#e8e8e8] transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-6 py-3 border-b border-[#e8e8e8]">
          <div className="relative">
            <Search className="absolute start-3 top-1/2 -translate-y-1/2 text-[#8c8c8c]" size={16} />
            <input
              type="text"
              placeholder="Search events..."
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="w-full bg-[#e8e8e8]/50 border border-[#e8e8e8] rounded-lg py-2 ps-9 pe-4 text-sm focus:ring-2 focus:ring-[#000000] outline-none"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-3 space-y-2">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center gap-2">
              <p className="text-sm font-medium text-[#000000]">Failed to load events</p>
              <p className="text-xs text-[#8c8c8c]">{loadError}</p>
            </div>
          ) : filtered.length === 0 ? (
            <p className="text-center text-sm text-[#8c8c8c] py-8">No upcoming events found.</p>
          ) : (
            filtered.map(event => {
                ...
                <label
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors border ${
                    checked
                      ...
                      : 'border-[#e8e8e8] hover:bg-[#e8e8e8]/50'
                  }`}
                >
                  <input
                    type="checkbox"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm text-[#000000] truncate">{event.title}</div>
                    <div className="flex items-center gap-3 mt-0.5">
                      ...
                      {event.location_name && (
                        <span className="flex items-center gap-1 text-xs text-[#8c8c8c] truncate">
        ...
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-[#e8e8e8]">
          <button
            onClick={onClose}
            disabled={saving}
            className="px-4 py-2 text-sm font-medium text-[#000000] bg-[#e8e8e8] rounded-lg hover:bg-[#d0d0d0] transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            ...
            className="px-4 py-2 text-sm font-medium text-white bg-[#000000] rounded-lg hover:bg-accent transition-colors disabled:opacity-50 flex items-center gap-2"
          >
