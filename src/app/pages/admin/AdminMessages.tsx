      ...
      <div className="bg-card rounded-xl border border-warm-gray/50 shadow-sm overflow-hidden min-w-0">
        ...
        <div className="md:hidden divide-y divide-warm-gray/50">
          ...
          {!isLoading && !isError && items.length === 0 && (
            <div className="py-12 px-4 text-center text-muted-foreground text-sm">{t("admin.messages.none")}</div>
          )}
          {!isLoading && !isError &&
            items.map((m) => (
              <div key={m.id} className="p-4 space-y-3">
                  ...
                  <div className="flex items-start gap-2 min-w-0 flex-1">
                    <span className="shrink-0 pt-0.5">
                      {m.read_at ? (
                        ...
                        <Circle className="w-5 h-5 text-muted-foreground" aria-label={t("admin.messages.unread")} />
                      )}
                    </span>
                    <div className="min-w-0 flex-1">
                      <div className="font-medium text-ink-black text-sm break-words">{m.name}</div>
                      <div className="text-xs text-muted-foreground flex items-start gap-1 mt-1 break-all">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 text-sm rounded-lg border border-warm-gray hover:bg-muted"
                  >
                  ...
                  {!m.read_at && (
                    <button
                      ...
                      className="px-3 py-1.5 text-sm rounded-lg bg-primary text-primary-foreground hover:bg-accent disabled:opacity-50"
                    >
                ...
                <>
                  {[1, 2, 3].map((n) => (
                    <tr key={n} className="animate-pulse">
                      <td className="py-3 px-6"><div className="h-4 w-6 bg-muted rounded mx-auto" /></td>
                      <td className="py-3 px-6"><div className="h-4 max-w-[12rem] bg-muted rounded" /></td>
                      <td className="py-3 px-6"><div className="h-4 max-w-full bg-muted rounded" /></td>
                      <td className="py-3 px-6"><div className="h-4 w-20 bg-muted rounded" /></td>
                  ...
                  <tr key={m.id} className="hover:bg-muted/30 transition-colors">
                    <td className="py-3 px-6 align-middle text-center">
                      {m.read_at ? (
                        ...
                        <Circle className="w-5 h-5 text-muted-foreground mx-auto" aria-label={t("admin.messages.unread")} />
                      )}
                    </td>
                    ...
                    <td className="py-3 px-6 align-middle text-muted-foreground whitespace-nowrap tabular-nums text-xs lg:text-sm text-start">
                      {formatDate(m.created_at)}
                    </td>
                    <td className="py-3 px-6 align-middle text-end">
                      <div className="flex flex-wrap items-center justify-end gap-2">
                        <button
                          type="button"
                          onClick={() => setDetailId(m.id)}
                          className="px-2.5 py-1.5 text-xs lg:text-sm rounded-lg border border-warm-gray hover:bg-muted shrink-0"
                        >
                          {t("admin.messages.view")}
                        </button>
                        {!m.read_at && (
                          <button
                            ...
                            disabled={markRead.isPending}
                            onClick={() => handleMarkRead(m.id)}
                            className="px-2.5 py-1.5 text-xs lg:text-sm rounded-lg bg-primary text-primary-foreground hover:bg-accent disabled:opacity-50 shrink-0"
                          >
                            {t("admin.messages.mark_read")}
                          </button>
                        )}
          ...
          {selected && (
            <div className="space-y-3 text-sm">
              ...
              {!selected.read_at && (
                <button
                  type="button"
                  className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm hover:bg-accent"
                  onClick={() => {
